...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def expectimax(self, gameState, depth, agent):
        ...
        legalActions = gameState.getLegalActions(agent)
        for action in legalActions:
            succUtility = self.expectimax(gameState.generateSuccessor(agent, action), nextDepth, nextAgent)
            succUtilities.append(succUtility)
        ...
        if agent == 0:
            ...
            # At a min node, return the average
            avg = float(sum(succUtilities))/len(succUtilities)
            return avg
...
def betterEvaluationFunction(currentGameState):
    ...
    foodDistances = [manhattanDistance(pacmanPosition, food) for food in foodPositions]
    ...
    if activeGhostDistances:
        minActiveGhost = min(activeGhostDistances)
    else:
        minActiveGhost = -5
    ...
    if scaredGhostDistances:
        minScaredGhost = min(scaredGhostDistances)
